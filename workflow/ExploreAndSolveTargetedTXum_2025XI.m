...
SectionBreak()
disp(['SETTINGS for TARGETED ', upper(ECs.settings.modelType),' ',upper(ECs.settings.viscDep),' models:'])
...
ECs.avg.rho = 4000;                                                                     % density [kg m^-3]
...
ECs.pd.t = 5.543e9*ECs.conversion.yr2sec;                           % [s]
...
if ECs.settings.varyphiRum == 'y'
    ECs.range.T0 = [1336 1624] + ECs.conversion.degC2K; %[1269 1650]
    ECs.range.totalH2Omass = [1.25 3.50] .* ECs.pd.oceanmass; %[1.25 4.53]
    ECs.range.H2Opartition = [1.87 76.61] ./ 100; %[1.70 82.67]
    ECs.range.Xp = [1176 5390] .* ECs.conversion.ppm2massfrac; %[1125 7800]
    ECs.range.phiRum = [32.36 90.87] ./ 100; %[12.54 94.54]
    if ECs.settings.varyfR == 'y'
        ECs.range.fR0 = log10([91.22 790.76]); %[84.53 840.71]
        ECs.range.MagniCLinearScale = [0.65 0.80]; %[0.60 0.91]
    end
    ECs.range.fRGinMantle = [63.65 74.83] ./ 100; % [62.54 75.00]
    ECs.range.C_U235 = [0.09 0.18] .* ECs.conversion.ppm2massfrac ./ 1e3; %[0.09 0.18]
    ECs.range.C_U238 = [15.08 24.00] .* ECs.conversion.ppm2massfrac ./ 1e3; %[14.77 24.00]
    ECs.range.C_Th232 = [52.22 90.81] .* ECs.conversion.ppm2massfrac ./ 1e3; %[51.16 91.00]
    ECs.range.C_K40 = [18.55 36.00] .* ECs.conversion.ppm2massfrac ./ 1e3; %[17.00 36.00]
    ECs.range.E = [262 448] .* 1e3; %[260 450]
    ECs.range.eta_pd = log10([1.32e20 6.16e21]); %[1.25e20 6.60e21]
else
    ECs.range.T0 = [1382 1649] + ECs.conversion.degC2K; %[1312 1650]
    ECs.range.totalH2Omass = [1.25 1.36] .* ECs.pd.oceanmass; %[1.25 1.81]
    ECs.range.H2Opartition = [.246 9.568] ./ 100; %[.243 9.747]
    ECs.range.Xp = [1422 7106] .* ECs.conversion.ppm2massfrac; %[1393 7800] 
    ECs.range.fum = [17.46 45.87] ./ 100; %[5.81 46.38]
    if ECs.settings.varyfR == 'y'
        ECs.range.fR0 = log10([40.20 966.81]); %[38.47 1000]
        ECs.range.MagniCLinearScale = [0.65 0.90]; %[0.6 1.0]
    end
    ECs.range.fRGinMantle = [60.91 75.00] ./ 100; %[58.58 75.00]
    ECs.range.C_U235 = [0.09 0.18] .* ECs.conversion.ppm2massfrac ./ 1e3; %[0.09 0.18]
    ECs.range.C_U238 = [14.98 23.78] .* ECs.conversion.ppm2massfrac ./ 1e3; %[14.90 24.00]
    ECs.range.C_Th232 = [54.09 90.38] .* ECs.conversion.ppm2massfrac ./ 1e3; %[53.96 91.00]
    ECs.range.C_K40 = [22.35 35.87] .* ECs.conversion.ppm2massfrac ./ 1e3; %[21.92 36.00]
    ECs.range.E = [261 410] .* 1e3; %[260 450]
    ECs.range.eta_pd = log10([1.23e20 6.61e21]); %[1.11e20 8.98e21]
end
...
parfor i = 1:ECs.MC.iterations
	...
	solved(i).MC.Tpd = interp1(solved(i).TS.t, solved(i).TS.T, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');

        % -- upper mantle water concentration [ppm]
        solved(i).MC.Xum0 = solved(i).TS.Xum(1);
	solved(i).MC.Xumpd = interp1(solved(i).TS.t, solved(i).TS.Xum, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');
	solved(i).MC.fH2Oumpd = interp1(solved(i).TS.t, solved(i).TS.fH2Oum, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');
        
        % -- plate water concentration [ppm]
	solved(i).MC.Xppd = interp1(solved(i).TS.t, solved(i).TS.Xp, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');

        % -- regassing efficiency [%] 
	solved(i).MC.fRpd = interp1(solved(i).TS.t, solved(i).TS.fR, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');
        
        % -- plate thickness [km]
	solved(i).MC.deltapd = interp1(solved(i).TS.t, solved(i).TS.delta, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');

        % -- plate speed [cm/yr]
	solved(i).MC.upd = interp1(solved(i).TS.t, solved(i).TS.u, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');

        % -- total heat flow out [TW]
	solved(i).MC.Qspd = interp1(solved(i).TS.t, solved(i).TS.Qs, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');
        
        % -- calculated upper mantle viscosity [Pa s]
	solved(i).MC.eta_umpd = interp1(solved(i).TS.t, solved(i).TS.eta_um, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');

        % -- present-day surface ocean mass [OM]
	solved(i).MC.oceanmasspd = interp1(solved(i).TS.t, solved(i).TS.oceanmass, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');

        % -- present-day R - D
	solved(i).MC.RminusDpd = interp1(solved(i).TS.t, solved(i).TS.RminusD, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');
        
        % -- present-day Urey Ratio
	solved(i).MC.Ureypd = interp1(solved(i).TS.t, solved(i).TS.UreyRatio, LCs.pd.ageEarth .* LCs.conversion.sec2yr ./ 1e9, 'linear');
    end
end
toc
...
if strcmp('rev', ECs.settings.timeDirection)
    myplots.solved.labelsBestOf = {'t', 'T', 'OM', 'u', 'Qs', 'fR', 'RminusD', 'fH2Oum'};
else
    myplots.solved.labelsBestOf = {'T', 'OM', 'u', 'Qs', 'fH2Oum', 'Xum', 'RminusD', 'UreyRatio', 'fR'};
end
...
for j = 1:ECs.MC.iterations
	...
	% Xum
	if (solved(j).MC.Xumpd >= 50 && solved(j).MC.Xumpd <= 200)
	    myplots.solved.feasibleXum = myplots.solved.feasibleXum + 1;
	end
        % R-D
        if (solved(j).MC.RminusDpd >= 2.7e11 && solved(j).MC.RminusDpd <= 5.0e11)
            myplots.solved.feasibleRminusD = myplots.solved.feasibleRminusD + 1;
        end
        % Urey ratio
        if (solved(j).MC.Ureypd >= 0.12 && solved(j).MC.Ureypd <= 0.49)
            myplots.solved.feasibleUreyRatio = myplots.solved.feasibleUreyRatio + 1;
        end
	% fR
	if (solved(j).MC.fRpd >= 10 && solved(j).MC.fRpd <= 60)
	    myplots.solved.feasiblefR = myplots.solved.feasiblefR + 1;
	end
	myplots.successConditions = (solved(j).MC.Tpd >= ECs.range.Tpd(1)-ECs.conversion.degC2K && solved(j).MC.Tpd <= ECs.range.Tpd(end)-ECs.conversion.degC2K) && ...
				    ...
				    (solved(j).MC.Xumpd >= 50 && solved(j).MC.Xumpd <= 200) && ...	
           			    (solved(j).MC.RminusDpd >= 2.7e11 && solved(j).MC.RminusDpd <= 5.0e11) && ...
				    (solved(j).MC.Ureypd >= 0.12 && solved(j).MC.Ureypd <= 0.49) && ...	
				    (solved(j).MC.fRpd >= 10 && solved(j).MC.fRpd <= 60);
